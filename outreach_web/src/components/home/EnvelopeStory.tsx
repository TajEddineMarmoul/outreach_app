...
const CHECK_START = THREAD_START + THREAD_DURATION - CHECK_LEAD;
const EASE_IN_OUT = "ease-in-out";
const RECIPIENT_LEAD = 100;
const LENA_POP_TIME = 2550;
...
function envelopeArrivals(root: HTMLDivElement, mobile: boolean) {
  ...
  return people.map(({ id }) => {
    const bounds = root.querySelector(`.story-envelope--${id}`)!.getBoundingClientRect();
    // Compare in SVG coordinates so zoom, centered frames and screen size do
    // not change when an envelope appears along the same curve.
    ...
    const arrivalTime = THREAD_START + easeInOutTimeForProgress(closest.progress) * THREAD_DURATION;
    const synchronizedDelay = Math.max(THREAD_START, arrivalTime - RECIPIENT_LEAD);
    return { id, delay: id === "lena" ? Math.min(synchronizedDelay, LENA_POP_TIME) : synchronizedDelay };
  });
}
...
export default function EnvelopeStory() {
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      // Share the route's initial animation clock so the badge is fully visible
      // at 2.95s, then pops while the route draws its final 0.8s.
      animate(".story-delivered-icon", [{ opacity: 0 }, { opacity: 1 }], { duration: 1, delay: CHECK_START, easing: EASE_IN_OUT });
      animate(".story-delivered-icon", deliveredPop, { duration: CHECK_LEAD, delay: CHECK_START, easing: EASE_IN_OUT });
      animate(".story-delivered-text", [{ opacity: 0, transform: "translateX(-5px)" }, { opacity: 1, transform: "translateX(0)" }], { duration: 400, delay: CHECK_START, easing: EASE_IN_OUT });
    };
