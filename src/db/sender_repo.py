...
def upsert_sender(
    ...
    if existing:
        ...
        cursor = conn.execute(
            """
            INSERT INTO senders(email, display_name, token_path, connected_at, status, daily_cap, is_default, group_name)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (normalized, display_name, token_path, now, status, daily_cap, 1 if is_default else 0, "Default Group"),
        )
