...
def connect(db_path: str | Path | None = None) -> sqlite3.Connection | PGConnection:
    return _get_conn(db_path)


def init_db(db_path: str | Path | None = None) -> sqlite3.Connection | PGConnection:
    conn = connect(db_path)
    create_tables(conn)
    return conn


def _pg_column_type(t: str) -> str:
    if t == "INTEGER PRIMARY KEY AUTOINCREMENT":
        return "SERIAL PRIMARY KEY"
    return t


def _convert_ddl(sql: str, for_pg: bool) -> str:
    if not for_pg:
        return sql
...
def create_tables(conn: sqlite3.Connection | PGConnection) -> None:
    is_pg = _is_pg(conn)
    if is_pg:
